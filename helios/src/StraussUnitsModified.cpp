...
class KTensor {
...
#if DEBUG
            std::cout << "Krr = " << rr / 3.74e5
                << "\n\tKrr2 = " << krr_r / 3.74e5
                << "\nKphph = " << phph / 3.74e5
                << "\nKrph = " << rph / 3.74e5
                << "\n\tKrph2 = " << krph_r / 3.74e5
                << "\nKthth = " << thth / 3.74e5
                << "\ndKrr_dr = " << dKrr_dr / 3.74e5
                << "\n\tdelta_r = " << deltar
                << "\ndKrph_dr = " << dKrph_dr / 3.74e5 << std::endl;
#endif
    ...
    private:
        KTensor& updateKElements(double rc, double thc, double Pc) {
            ...
            double kpar = (beta * 749.5 / 3) * lambda0 * (1 + rc / rRefLambda) * (Pc >= P0? Pc / P0 : 1);
...
#if DEBUG
            std::cout << "kpar = " << kpar / 3.74e5 << std::endl;
            std::cout << "kperp = " << kperp / 3.74e5 << std::endl;
...
void updateVd() {
    ...
    double rL = P/749.5 * 1 / Bmag;

    if (d <= 2 * rL) {
        double hcsDriftFact = Ac * qSign * (0.457 - 0.412 * d / rL + 0.0915 * pow(d / rL, 2)) * beta * 749.5;
        vdr += hcsDriftFact * sinPsi;
        vdph += hcsDriftFact * cosPsi;
#if DEBUG
        std::cout << "hcsDriftFact = " << hcsDriftFact / 3.74e5 << std::endl;
#endif
    }
...
#if DEBUG
    std::cout << "vdCoeff = " << vdCoeff / 3.74e5
        << "\nvdr = " << vdr / 3.74e5
        << "\nvdth = " << vdth / 3.74e5
        << "\nvdph = " << vdph / 3.74e5 << std::endl;
#endif
}
...
void updateVars() {
    ...
    std::cout << "tanPsi = " << Omega1*protime*(r - rSun)*sin(th)/Vsw << std::endl;
    std::cout << "beta = " << P / sqrt(P*P + m*m) << std::endl;
#endif
}
...
#if DEBUG
    std::cout << "dr_ds = " << dr_ds / 3.74e5 << std::endl;
    std::cout << "dr_dWr = " << dr_dWr << std::endl;
    std::cout << "dr_dWph = " << dr_dWph << std::endl;
    std::cout << "dth_ds = " << dth_ds / 3.74e5 << std::endl;
    std::cout << "dth_dWth = " << dth_dWth << std::endl;
    std::cout << "dph_ds = " << dph_ds / 3.74e5 << std::endl;
    std::cout << "dph_dWph = " << dph_dWph << std::endl;
    std::cout << "dE_ds = " << dek_ds / 3.74e5 << std::endl;
#endif
...
int main(int argc, char *argv[]) {
    ...
    if (argc == 4) {
        ...
        const std::string runDir("testrundata/");
