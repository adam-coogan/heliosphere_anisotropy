...
!	distance = abs(r*cos(theta))
	...
	IF(distance.LE.2.0*Larmor)THEN
		...
		vdr = vdr + pol_cycle*charge*(0.457 - 0.412*distance/Larmor + 0.0915*distance*distance/Larmor/Larmor)*BETA*750.0*FS*sinpsi*cos_beta
                vdphi = vdphi + pol_cycle*charge*(0.457 - 0.412*distance/Larmor + 0.0915*distance*distance/Larmor/Larmor)*BETA*750.0*FS*cospsi*cos_beta
		vdtheta = vdtheta + pol_cycle*charge*(0.457 - 0.412*distance/Larmor + 0.0915*distance*distance/Larmor/Larmor)*BETA*750.0*FS*sin_beta
...
!	First test point - REFLECT
	...
	CALL LINE_ELEMENT(r_,phi_,theta_,r_test,phi_test,alfa,Omega,L_test)
